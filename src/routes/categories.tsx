...
import { Plus, Folder, FolderPlus, ChevronDown, ChevronRight, Edit2, Trash2, Tag, Layers, RefreshCw } from 'lucide-react';
// Conexão direta com a tomada que você localizou no repositório!
...
function CategoriesComponent() {
  ...
  // 1. BUSCAR (FETCH) DADOS REAIS DO SUPABASE
  const fetchCategories = async () => {
    ...
    if (error) {
      ...
      // Injeta o atributo de controle visual isExpanded nas categorias pai
  ...
  // 2. SALVAR OU ATUALIZAR REGISTROS REAIS
  const handleSave = async () => {
    ...
    setShowModal(false);
    fetchCategories(); // Recarrega os dados atualizados direto do banco!
  };

  // 3. EXCLUIR REGISTRO REAL NO BANCO
  const handleDelete = async (id: string) => {
    const confirm = window.confirm("Deseja mesmo excluir esta categoria? Lançamentos associados podem ficar sem categoria.");
    if (!confirm) return;
    ...
    if (error) {
      ...
      fetchCategories(); // Recarrega a lista limpa
    }
  };
  ...
  // Filtros de Hierarquia baseados nas regras do PRD
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/[0.06] pb-6">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">
            Dados conectados diretamente à infraestrutura do seu Supabase.
          </p>
        </div>
        <button 
          onClick={handleOpenCreate}
          className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-lg transition-all flex items-center justify-center gap-2 self-start sm:self-center"
        >
      ...
      {/* Árvore de Listagem ou Loading State */}
                ...
                <div key={parent.id} className="border border-zinc-900 rounded-xl bg-zinc-900/20 overflow-hidden">
                  <div className="flex items-center justify-between p-4 hover:bg-white/[0.01] transition-colors">
                    ...
                    <div className="flex items-center space-x-1">
                      <button onClick={() => handleOpenEdit(parent)} className="text-zinc-500 hover:text-white p-1.5 rounded transition-colors"><Edit2 className="w-4 h-4" /></button>
                      <button onClick={() => handleDelete(parent.id)} className="text-zinc-600 hover:text-rose-400 p-1.5 rounded transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                    ...
                    <div className="bg-black/20 border-t border-zinc-900/50 divide-y divide-zinc-900/30 pl-10 pr-4">
                      {children.map((child) => (
                        <div key={child.id} className="flex items-center justify-between py-3 hover:bg-white/[0.01] transition-colors">
                          ...
                          <div className="flex items-center space-x-1">
                            <button onClick={() => handleOpenEdit(child)} className="text-zinc-500 hover:text-white p-1 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                            <button onClick={() => handleDelete(child.id)} className="text-zinc-600 hover:text-rose-400 p-1 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div className="bg-zinc-900 border border-white/[0.08] max-w-sm w-full rounded-2xl p-6 space-y-4 shadow-2xl">
            <div>
              <h3 className="text-lg font-bold text-white">
                {modalMode === 'create' ? 'Nova Categoria Real' : 'Editar Categoria Real'}
              </h3>
            </div>
            ...
            <div className="space-y-3.5">
              <div className="space-y-1">
                ...
                <input 
                  ...
                  placeholder="Ex: Assinaturas de Softwares"
                  className="w-full bg-zinc-950 border border-zinc-800 outline-none text-sm rounded-xl px-3 py-2 text-zinc-100"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <select 
                  ...
                  className="w-full bg-zinc-950 border border-zinc-800 outline-none text-sm rounded-xl px-3 py-2 text-zinc-300"
                >
            ...
            <div className="flex space-x-3 pt-2">
              ...
              <button onClick={handleSave} className="w-full py-2 bg-indigo-600 text-white text-xs font-bold rounded-xl">Salvar no Supabase</button>
            </div>
          </div>
        </div>
      )}
