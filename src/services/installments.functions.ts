...
export const listInstallmentPurchases = createServerFn({ method: "GET" }).handler(
  async (): Promise<InstallmentPurchaseDTO[]> => {
    ...
    return ((data ?? []) as unknown as Array<{
      ...
      const remaining_amount = remainingItems
        .reduce((acc, i) => acc + Math.round(Number(i.amount) * 100), 0);
...
export const listLoans = createServerFn({ method: "GET" }).handler(
  async (): Promise<LoanDTO[]> => {
    const { getSupabaseAdmin } = await import("@/lib/supabase/client.server");
    const sb = getSupabaseAdmin();
    const { data, error } = await sb
      .from("loans")
    ...
    if (error) throw new Error(error.message);
    return ((data ?? []) as unknown as Array<LoanDTO & {
      accounts?: { name?: string | null } | null;
